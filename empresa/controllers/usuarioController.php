<?php
   class usuarioController extends controllers
   {
      ...
      public function usuario($rutabase,$DatosConf)
      {
        $ModeloIndex=$this->loadModel('usuario');
        $vistaIndex=$this->loadView('usuario');
     		$vistaIndex->renderizar('usuario','usuario',$rutabase,$DatosConf,$this->parametros);
      }
...
public function comprobar ($NomUser,$DirUser,$CelUser,$EmailUser,$Cont1User,$Cont2User,$rutabase,$params)
      {
        $ModeloIndex=$this->loadModel('usuario');
        $vistaIndex=$this->loadView('usuario');
         ...
         if (empty($VarEmail)) {
          $this->validacion['user'] = true;
           ...
           if ($this->validacion['user']==true and $this->validacion['contra']==true and $this->validacion['email'] = true) {
              ...
              if ($envio==true) {
                 $post=$this->loadModel('usuario');
                 $ModeloIndex->InsertarUsuario($NomUser,$EmailUser,$Pass,$RolUser,$CodUser,$CelUser,$DirUser,$ActUser);
                 $this->parametros['contra'] = true;
                 $vistaIndex->renderizar('codigo','codigo',$rutabase,$params,$this->parametros);
              }
              #############################################################################################
           ...
           ######################################################################################################
            if ($envio!=true) {
              //$this->_view->renderizar('usuario','usuario',$rutabase,'hola');
              $vistaIndex->renderizar('usuario','usuario',$rutabase,$params,$this->parametros);
...
/***************************************Cierre metodo comprobacion*********************************************/
      ...
      public function validar($rutabase,$params=[])
      {
      if (isset($_POST["Nombre"])) {
        ...
        $this->comprobar($NomUser,$DirUser,$CelUser,$EmailUser,$Cont1User,$Cont2User,$rutabase,$params);
       //$s=$params['DB_CHAR'];
       //echo $s;
